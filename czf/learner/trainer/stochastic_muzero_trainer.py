...
class StochasticMuZeroTrainer(Trainer):
    ...
    def __init__(self, config, checkpoint_path, model_path, log_path, model_name,
        ...
        if restore_checkpoint_path:
            print('Restore from', restore_checkpoint_path)
            with open(restore_checkpoint_path, 'rb') as model_blob:
        ...
        optimizer_name = config['learner']['optimizer'].get('name', 'SGD')
        ...
        if optimizer_name == 'SGD':
            ...
            self._optimizer = torch.optim.Adam(
                self._model.parameters(),
                lr=config['learner']['optimizer']['learning_rate'],
                betas=config['learner']['optimizer']['betas'],
                eps=config['learner']['optimizer']['eps'],
                weight_decay=config['learner']['optimizer']['weight_decay'],
            )
        if restore_checkpoint_path:
    ...
    def _train(self, dataloader, replay_buffer):
        ...
        for rollout in dataloader:
            print(f'[train] dataloader with time {time.time() - start:.3f}')
            start = time.time()
            # tensor
